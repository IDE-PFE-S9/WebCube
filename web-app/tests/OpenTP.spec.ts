...
test('Open a TP', async ({ page }) => {
	...
	// verify that there is a button with the name 'src'
	const button = page.getByRole('button', { name: 'src' });
	await expect(button).toBeVisible();
});

test('Open a TP when a TP is already opened', async ({ page }) => {
	await page.goto('http://localhost:5173/');
	await page.context().addCookies([
		{
			name: 'apiJWT',
	...
	await page.getByRole('button', { name: 'TP-1' }).click();
	...
	// verify that there is a button with the name 'src'
	const button2 = page.getByRole('button', { name: 'src' });
