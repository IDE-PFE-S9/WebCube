...
test('Compile Code', async ({ page }) => {
    ...
    await expect(page.getByLabel('Code compilé !')).toBeVisible();

    await expect(page.getByText('Hello World').first()).toBeVisible();
    await expect(page.getByText('Program exited with code 0')).toBeVisible();

    await page.locator('.clear-button').click();

    await expect(page.getByText('Hello World').first()).not.toBeVisible();
    await expect(page.getByText('Program exited with code 0')).not.toBeVisible();
});


test('Fail compile Code', async ({ page }) => {
    await page.goto('http://localhost:5173/');
    await page.context().addCookies([
        {
            name: 'apiJWT',
    ...
    await page.getByRole('button', { name: 'tp', exact: true }).first().click();
    await page.getByRole('button', { name: 'Main.java', exact: true }).click();

    await page.getByLabel('Editor content;Press Alt+F1 for Accessibility Options.').fill('h');

    await page.getByRole('button', { name: 'Sortie' }).click();

    // save file
    await page.locator('.save-button').first().click();
    await expect(page.getByLabel('Travail enregistré !')).toBeVisible();

    await page.locator('.run-button').first().click();

    await page.waitForTimeout(1000);

    await expect(page.getByLabel('Compilation échouée')).toBeVisible();

    await expect(page.getByText('Compilation failed with exit code 1')).toBeVisible();
