...
test('Fail download archive', async ({ page }) => {
    ...
    // verify popup
    await expect(page.getByLabel('Téléchargement échoué')).toBeVisible();
});


test('Download archive', async ({ page }) => {
    await page.goto('http://localhost:5173/');
    await page.context().addCookies([
        {
            name: 'apiJWT',
    ...
    await expect(page.getByLabel('Archive téléchargée')).toBeVisible();
});