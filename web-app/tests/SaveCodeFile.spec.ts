import { test, expect } from '@playwright/test';

let apiJWT =
	'eyJhbGciOiJIUzUxMiJ9..CNUsQd5JoLrFDKnzcuLUb4Akobr0nqsqSbFfg1x1oz3Of30A4MUMBMJBXHpgaKNNAruBxkRuH--FCf8Vc4cgLw';

test('Edit a code file and save', async ({ page }) => {
    await page.goto('http://localhost:5173/');
    await page.context().addCookies([
        {
            name: 'apiJWT',
            value: apiJWT,
            url: 'http://localhost:5173'
        },
        {
            name: 'azureJWT',
            value:
                'eyJ0eXAiOiJKV1QiLCJub25jZSI6InVZWEdmZHlwdzJsaDBIUkVZdmVZMm1QRG1CQ2JURGRyWVhPWGNnYkZoS28iLCJhbGciOiJSUzI1NiIsIng1dCI6IlQxU3QtZExUdnlXUmd4Ql82NzZ1OGtyWFMtSSIsImtpZCI6IlQxU3QtZExUdnlXUmd4Ql82NzZ1OGtyWFMtSSJ9..noDGHpqefjNizz9AFXK2vJU-HNTdRYbESSpbG4mJIBm6g3nrMDLpQ7NxJPw7do2n91hdGrGNl75yhZtXkaW7XBGsVUa-awFFYcBLfNO7A8b3hUbbUF8QYlcPETEQTvMq0YuqP_DGApFMb5ljBe41mWUQUzxB8W0fCmVis9lWpcsTPWb83SlyPtWha80otG-LqXp329mphucs5uJpcjdJXIysB3WDEU4kJQUh3dVnQ7URp5TfYO-EuqJw0vf5TShuOoAa7EJYF6E6IxlgGtm4FPr9J5p1xmPPWK7cCoco0kOaQr5g6Ypcc7trPCJmIRA5emexOEEFNL65EhtlCfAOdA',
            url: 'http://localhost:5173'
        }
    ]);

    // reload page
    await page.reload();

    await page.waitForTimeout(1000);

    // Open the TP
    await page.getByRole('button', { name: 'TP-TEST' }).click();

    await page.waitForTimeout(1000);

    await page.getByRole('button', { name: 'TP-TEST' }).click();

    await page.getByRole('button', { name: 'src' }).click();
    await page.getByRole('button', { name: 'tp', exact: true }).first().click();
    await page.getByRole('button', { name: 'Main.java', exact: true }).click();

    await page.locator('.view-lines > div:nth-child(12)').click();
    await page.getByLabel('Editor content;Press Alt+F1 for Accessibility Options.').fill('}\nHello World');

    // save file
    await page.getByRole('button', { name: 'Sortie' }).click();
    await page.locator('.save-button').first().click();

    // verfiy popup
    await expect(page.getByLabel('Travail enregistré !')).toBeVisible();
    
    // save file
    await page.getByRole('button', { name: 'Sortie' }).click();
    await page.locator('.save-button').first().click();

    // reload page
    page.reload();

    // Open the TP
    await page.getByRole('button', { name: 'TP-TEST' }).click();

    await page.waitForTimeout(1000);

    await page.getByRole('button', { name: 'TP-TEST' }).click();

    await page.getByRole('button', { name: 'src' }).click();
    await page.getByRole('button', { name: 'tp', exact: true }).first().click();
    await page.getByRole('button', { name: 'Main.java', exact: true }).click();

    let text = page.locator('.view-lines > div:nth-child(12)').getByText('Hello World', { exact: true });

    await expect(text).toBeVisible();


    // TODO: FIX HERE
    // delete text "Hello World"
    await page.getByRole('code').locator('div').filter({ hasText: 'Hello World' }).nth(4).click({
      clickCount: 3
    });

    await page.keyboard.press('Delete');

    await page.getByRole('button', { name: 'Sortie' }).click();
    await page.locator('.save-button').first().click();
});



test('Fail Save code file', async ({ page }) => {
    await page.goto('http://localhost:5173/');
    await page.context().addCookies([
        {
            name: 'apiJWT',
            value: apiJWT,
            url: 'http://localhost:5173'
        },
        {
            name: 'azureJWT',
            value:
                'eyJ0eXAiOiJKV1QiLCJub25jZSI6InVZWEdmZHlwdzJsaDBIUkVZdmVZMm1QRG1CQ2JURGRyWVhPWGNnYkZoS28iLCJhbGciOiJSUzI1NiIsIng1dCI6IlQxU3QtZExUdnlXUmd4Ql82NzZ1OGtyWFMtSSIsImtpZCI6IlQxU3QtZExUdnlXUmd4Ql82NzZ1OGtyWFMtSSJ9..noDGHpqefjNizz9AFXK2vJU-HNTdRYbESSpbG4mJIBm6g3nrMDLpQ7NxJPw7do2n91hdGrGNl75yhZtXkaW7XBGsVUa-awFFYcBLfNO7A8b3hUbbUF8QYlcPETEQTvMq0YuqP_DGApFMb5ljBe41mWUQUzxB8W0fCmVis9lWpcsTPWb83SlyPtWha80otG-LqXp329mphucs5uJpcjdJXIysB3WDEU4kJQUh3dVnQ7URp5TfYO-EuqJw0vf5TShuOoAa7EJYF6E6IxlgGtm4FPr9J5p1xmPPWK7cCoco0kOaQr5g6Ypcc7trPCJmIRA5emexOEEFNL65EhtlCfAOdA',
            url: 'http://localhost:5173'
        }
    ]);

    // reload page
    await page.reload();

    await page.waitForTimeout(1000);

    await page.getByRole('button', { name: 'Sortie' }).click();
    await page.locator('.save-button').first().click();

    // verify popup
    await expect(page.getByLabel('Travail non enregistré')).toBeVisible();
});